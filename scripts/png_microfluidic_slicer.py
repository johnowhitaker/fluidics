...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--prime-mm", type=float, default=5.0)
...
def add_port(path: Path2D, args: argparse.Namespace) -> Path2D:
    if args.port == "none" or not path.points:
        return path
    start = path.points[0]
    return Path2D([(start[0] + args.port_length, start[1] + args.port_length), start] + path.points)
